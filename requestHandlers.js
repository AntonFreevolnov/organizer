...
function upload(response, request) {
  ...
  form.parse(request, function(error, fields, files) {
    ...
    /* Возможна ошибка в Windows: попытка переименования уже существующего файла */
    fs.rename(files.upload.path, "tmp/test.png", function(err) {
    if (err) { fs.unlink("/tmp/test.png");
...
function show(response) {
  ...
  fs.readFile("tmp/karpov.png", "binary", function(error, file) {
    if(error) {
